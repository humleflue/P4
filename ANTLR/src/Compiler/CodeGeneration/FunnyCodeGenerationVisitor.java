...
public class FunnyCodeGenerationVisitor extends BuffBaseVisitor<ReturnValue> {
    ...
    /**
     * Creates the map between parameter names and their actual values that is needed by this visitor
     * @param functionName The name of the function being called
     * @param exprParams The ExprParams node from the FuncCall. Used to get the values
     * @return Returns a map between parameter names and ReturnValues
     */
    ...
    public ReturnValue<?> visitExprBinaryOp(ExprBinaryOpContext ctx) {
        return switch (ctx.op.getType()) {
            case DIVIDE -> new ReturnValue<>((Double)visit(ctx.left).value / (Double)visit(ctx.right).value, NUMTYPE);
            case MULTIPLY -> new ReturnValue<>((Double)visit(ctx.left).value * (Double)visit(ctx.right).value, NUMTYPE);
            case PLUS -> new ReturnValue<>((Double)visit(ctx.left).value + (Double)visit(ctx.right).value, NUMTYPE);
            case MINUS -> new ReturnValue<>((Double)visit(ctx.left).value - (Double)visit(ctx.right).value, NUMTYPE);
            case LOGLESS -> new ReturnValue<>((Double)visit(ctx.left).value < (Double) visit(ctx.right).value, BOOLTYPE);
            case LOGGREATER -> new ReturnValue<>((Double)visit(ctx.left).value > (Double) visit(ctx.right).value, BOOLTYPE);
            case LOGLESSOREQ -> new ReturnValue<>((Double)visit(ctx.left).value <= (Double) visit(ctx.right).value, BOOLTYPE);
            case LOGGREATEROREQ -> new ReturnValue<>((Double)visit(ctx.left).value >= (Double) visit(ctx.right).value, BOOLTYPE);
            case LOGEQ -> new ReturnValue<>(visit(ctx.left).value.equals(visit(ctx.right).value), BOOLTYPE);
            case LOGNOTEQ -> new ReturnValue<>(!visit(ctx.left).value.equals(visit(ctx.right).value), BOOLTYPE);
            case LOGAND -> new ReturnValue<>((Boolean) visit(ctx.left).value && (Boolean) visit(ctx.right).value, BOOLTYPE);
            case LOGOR -> new ReturnValue<>((Boolean) visit(ctx.left).value || (Boolean) visit(ctx.right).value, BOOLTYPE);
