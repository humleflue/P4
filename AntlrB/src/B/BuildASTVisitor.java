...
public class BuildASTVisitor extends BBaseVisitor<Node>{
    ...
    private Node visitExpr(BParser.ExprContext expr) {
        if (IsLogOr(expr))
            return visitLogOr((BParser.LogOrContext) expr);
        else
            return visitLog((BParser.LogContext) expr);
    }

    boolean IsLogOr(BParser.ExprContext ctx){
        return ctx instanceof BParser.LogOrContext;
    }
    ...
    public Node visitStmt(BParser.StmtContext ctx) {
        if (IsLogOr(ctx.expr()))
            return new Stmt(visitLogOr((BParser.LogOrContext) ctx.expr()));
        else {
    ...
    @Override
    public Node visitLogOr(BParser.LogOrContext ctx) {
        if (IsLogAnd(ctx.left)) {
            if (IsLogOr(ctx.right))
                return new LogOr(visitLogAnd((BParser.LogAndContext) ctx.left),
                        visitLogOr((BParser.LogOrContext) ctx.right));
            else
                return new LogOr(visitLogAnd((BParser.LogAndContext) ctx.left),
            ...
            if (IsLogOr(ctx.right))
                return new LogOr(visitLog2((BParser.Log2Context) ctx.left),
                        visitLogOr((BParser.LogOrContext) ctx.right));
            else
                return new LogOr(visitLog2((BParser.Log2Context) ctx.left),
                        visitLog((BParser.LogContext) ctx.right));
    ...
    boolean IsLogAnd(BParser.LogexprContext ctx){
        return ctx instanceof BParser.LogAndContext;
    }

    @Override
    public Node visitLog(BParser.LogContext ctx) {
        if (IsLogAnd(ctx.logexpr()))
            return visitLogAnd((BParser.LogAndContext) ctx.logexpr());
        else
            return visitLog2((BParser.Log2Context) ctx.logexpr());
    }

    @Override
    public Node visitLogAnd(BParser.LogAndContext ctx) {
        if (IsLogEqualsOp(ctx.left)){
            if (IsLogAnd(ctx.right))
                return new LogAnd(visitLogEqualsOp((BParser.LogEqualsOpContext) ctx.left),
                        visitLogAnd((BParser.LogAndContext) ctx.right));
            else
                    return new LogAnd(visitLogEqualsOp((BParser.LogEqualsOpContext) ctx.left),
            ...
            if (IsLogAnd(ctx.right))
                return new LogAnd(visitLog3((BParser.Log3Context) ctx.left),
                        visitLogAnd((BParser.LogAndContext) ctx.right));
            else
                return new LogAnd(visitLog3((BParser.Log3Context) ctx.left),
                        visitLog2((BParser.Log2Context) ctx.right));
